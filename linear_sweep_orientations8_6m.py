...
# sonar sweep params
# eps should be 50% of waypoint distance 
SONAR_DIST = 2.2 # meters
# threshold for waypoint arrival
CAPTURE_STEP = 0.06 # meters per step
WAYPOINT_DIST = 0.5
EPS = WAYPOINT_DIST * 0.5
...
def main():
    ...
    with holoocean.make(scenario_cfg=config, verbose=True, show_viewport=True) as env:
        ...
        # ================= DEBUG CAMERA VIEW =================
                ...
                u = -np.array([math.cos(yaw_object_rad), math.sin(yaw_object_rad), 0.0], dtype=float)  # backwards
                u[2] = 0.0
                ...
                while sdist <= SONAR_DIST + 1e-9:
                    g = object_pose.copy()
                    g[0] = object_pos[0] + sdist * u[0]
                    g[1] = object_pos[1] + sdist * u[1]
                    g[2] = object_pos[2]
                    g[3] = 0.0
                    g[4] = 0.0
                    g[5] = yaw_object
                ...
                if dist_since_save >= CAPTURE_STEP and len(sonar_imgs) < 24:
                    ...
                    if latest_sonar_frame is not None:
                        ...
                        object = "small_crate"
                        sonar_path_jpg = os.path.join(main_path,f"{object}_only","6m_deep","sonar", dir_name,f"FLSc_{number}.jpg")
                       # sonar_path_npy = os.path.join(main_path,"cluster","objects_inputRGB", dir_name,f"FLSc_{number}.npy")
                        ...
                        # saving sonar image .jpg
                        img = save_jpg(fig, ax, plot, latest_sonar_frame_gain, (1500,1500), sonar_path_jpg)
                        sonar_imgs.append(img)
                        
                        print(f"\t[Orientation {real_idx+1}], Saved Img {images_captured} (Step: {dist_since_save:.3f}m)")
                ...
                if wp_idx >= len(sweep_waypoints):
                        ...
                        sonar_path_npy = os.path.join(main_path, "cluster", "objects_inputRGB", f"{object}_only_6m_deep_orient{real_idx+1}.npy")
